...
window.addEventListener('load', function () {
    ...
    var aufnahme: string[] = ["assets/kick.mp3", "assets/snare.mp3", "assets/hihat.mp3"];
    var index: number = 0;
    var soundIntervall: number;
    ...
    function playSample(kombi: string): void {
        if (micon) {
            aufnahme.push(kombi);
        }
        const ton: HTMLAudioElement = new Audio(kombi);
        ton.play();
    }



    function plystpBtn(): void {
        var psbutton: HTMLElement = document.querySelector("#play");
        if (psbutton.getAttribute("class") == "fas fa-play") {
            psbutton.setAttribute("class", "fas fa-stop");
            plyBtn();
    ...
    document.querySelector("#micro").addEventListener("click", recording);
    document.getElementById("delete").addEventListener("click", function (): void { aufnahme = []; });
});
